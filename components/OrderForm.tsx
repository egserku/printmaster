          ...
          <div className="animate-in slide-in-from-left-4 duration-400">
            ...
            <div className="flex flex-wrap gap-2">
              {productType === ProductType.HOODIE ? 
                [HoodieType.KANGAROO, HoodieType.ZIP, HoodieType.SWEATER].map(modelType => (
                  <button key={modelType} onClick={() => setFormData({ ...formData, hoodieType: modelType })} className={`px-6 h-12 rounded-2xl font-black text-[11px] uppercase border-2 transition-all ${formData.hoodieType === modelType ? 'border-indigo-600 bg-indigo-50 text-indigo-700' : 'border-gray-50 bg-gray-50 text-gray-300'}`}>{t(`models.${modelType.toLowerCase()}`)}</button>
                )) :
                [CapType.BASEBALL, CapType.SNAPBACK, CapType.BEANIE].map(modelType => (
                  <button key={modelType} onClick={() => setFormData({ ...formData, capType: modelType })} className={`px-6 h-12 rounded-2xl font-black text-[11px] uppercase border-2 transition-all ${formData.capType === modelType ? 'border-indigo-600 bg-indigo-50 text-indigo-700' : 'border-gray-50 bg-gray-50 text-gray-300'}`}>{t(`models.${modelType.toLowerCase()}`)}</button>
                ))
              }
            </div>
          </div>
